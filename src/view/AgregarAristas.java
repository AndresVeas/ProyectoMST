...
public class AgregarAristas extends JFrame {
    ...
    private void cargarResumenNodosDummy(JPanel container) {
        // Simulamos que tenemos un Mapa<Índice, Nombre> o una Lista de objetos Nodo
        String[] nodosSimulados = {
                "0. Nodo Inicial (A)",
                "1. Nodo Intermedio (B)",
                "2. Nodo Destino (C)",
                "3. Otro Nodo (D)",
                "4. Nodo Extra (E)"
                // Agrega más aquí para probar el scroll del resumen
        };

        for (String nodoInfo : nodosSimulados) {
            JLabel itemLabel = new JLabel("•  " + nodoInfo);
    ...
    private void addEdgeField(boolean isFirstExample) {
        ...
        txtInicio.setMaximumSize(new Dimension(Integer.MAX_VALUE, 45)); // Usar espacio disponible

        // Campo Destino
        RoundedNumberField txtDestino = new RoundedNumberField(phDestino);
        txtDestino.setMaximumSize(new Dimension(Integer.MAX_VALUE, 45));

        // Campo Peso
        RoundedNumberField txtPeso = new RoundedNumberField(phPeso);
        txtPeso.setMaximumSize(new Dimension(100, 45)); // Peso un poco más pequeño
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> new AgregarAristasApp().setVisible(true));
    }
}